...
event <- evt %>%
  ...
  mutate(
    eventID = if_else(
      str_count(transect_name, fixed("_")) == 1,
      paste0(transect_name, "_ukn"),
      transect_name
    ),
...
event_core <- event_hierarchical %>%
  select(any_of(dwc_event_fields), dynamicProperties) %>%
  distinct() %>%
  relocate(eventID, parentEventID, eventType)  # put these 3 fields first
...
eco <- event %>%
  ...
  rename(
    eventDurationValue = transect_time_min,
...
ac <- occ %>%
  ...
  left_join(event_hierarchical %>% select(eventID, campaign, parentEventID, rovID, verbatimLocality, dynamicProperties), by = "eventID") %>%
  select(eventID, sourceFileName, file_path, relative_path, campaign, parentEventID, rovID, verbatimLocality, dynamicProperties) %>%
  distinct() %>% 
  filter(!is.na(eventID), eventID != "") %>%
  mutate(
    `dc:type` = "StillImage",
...
# write files
write_tsv(event_core, here("data", "03_processed", "event.txt"), na = "")
write_tsv(eco, here("data", "03_processed", "humboldt.txt"), na = "")

# occurrence and audiovisual extension will be generated by upload-media-zenodo.R
